...
struct ext4_sblock {
	...
	/* For Oxbow's journaling and staging */
	uint64_t ox_journal_sb; /* background journal superblock */
	uint32_t ox_nr_journal_blocks;
	uint64_t ox_staging_sb; /* staging superblock */
	uint32_t ox_nr_staging_blocks;

	uint32_t padding[76]; /* Padding to the end of the block
			       * The total size of the superblock is 1024 bytes (EXT4_SUPERBLOCK_SIZE).
			       */
