...
void ext4_extent_tree_init(struct ext4_inode_ref *inode_ref)
{
    ...
    ext4_mark_inode_dirty(inode_ref);
    
    // Print extent tree after initialization
    ext4_print_extent_tree(inode_ref, "TREE_INIT");
}
...
// Helper function to find filename by inode number in a directory
static int ext4_find_filename_by_inode(struct ext4_inode_ref *dir_ref, uint32_t target_inode,
					char *filename, size_t max_len)
{
	struct ext4_fs *fs = dir_ref->fs;
	struct ext4_sblock *sb = &fs->sb;
	uint32_t iblock = 0;
	ext4_fsblk_t fblock;
	uint32_t block_size = ext4_sb_get_block_size(sb);
	uint64_t inode_size = ext4_inode_get_size(sb, dir_ref);
	uint32_t total_blocks = (uint32_t)(inode_size / block_size);
	int rc;

	// Walk through all data blocks in directory
	for (iblock = 0; iblock < total_blocks; ++iblock) {
		// Load block address
		rc = ext4_fs_get_inode_dblk_idx(dir_ref, iblock, &fblock, false);
		if (rc != EOK)
			continue;

		// Load data block
		struct ext4_block b;
		rc = ext4_trans_block_get(fs->bdev, &b, fblock, NULL);
		if (rc != EOK)
			continue;

		// Search through directory entries in this block
		struct ext4_dir_en *de = (struct ext4_dir_en *)b.data;
		uint8_t *addr_limit = b.data + block_size;
		...
		while ((uint8_t *)de < addr_limit) {
			...
			// Check if this entry matches our target inode
			if (ext4_dir_en_get_inode(de) == target_inode) {
				uint16_t name_len = ext4_dir_en_get_name_len(sb, de);
				if (name_len > 0 && name_len < max_len) {
					memcpy(filename, de->name, name_len);
					filename[name_len] = '\0';
...
// Function to get a filename for the given inode (for debugging purposes)
static void ext4_get_inode_path_info(struct ext4_inode_ref *inode_ref, char *path_info, size_t max_len)
{
	struct inode *vfs_inode = &inode_ref->vfs_inode;
	char filename[256] = {0};
	bool filename_found = false;
	
	// Initialize with basic inode info
	snprintf(path_info, max_len, "inode %u", inode_ref->index);
	
	// Add file type information
	if (vfs_inode->i_mode) {
		const char *type_str = "unknown";
		if (S_ISREG(vfs_inode->i_mode)) type_str = "file";
		else if (S_ISDIR(vfs_inode->i_mode)) type_str = "dir";
		else if (S_ISLNK(vfs_inode->i_mode)) type_str = "symlink";
		else if (S_ISCHR(vfs_inode->i_mode)) type_str = "chardev";
		else if (S_ISBLK(vfs_inode->i_mode)) type_str = "blockdev";
		else if (S_ISFIFO(vfs_inode->i_mode)) type_str = "fifo";
		else if (S_ISSOCK(vfs_inode->i_mode)) type_str = "socket";
		
		char temp[512];
		snprintf(temp, sizeof(temp), "%s (%s, size=%zu)", path_info, type_str, vfs_inode->i_size);
		strncpy(path_info, temp, max_len - 1);
		path_info[max_len - 1] = '\0';
	}
	
	// Try to find the actual filename
	if (inode_ref->index == EXT4_ROOT_INO) {
		snprintf(filename, sizeof(filename), "/");
		filename_found = true;
	} else {
		// For non-root inodes, try to find filename in parent directories
		// This is a simplified approach - we'll search from root directory
		struct ext4_inode_ref root_ref;
		if (ext4_fs_get_inode_ref(inode_ref->fs, EXT4_ROOT_INO, &root_ref) == EOK) {
			if (ext4_find_filename_by_inode(&root_ref, inode_ref->index, filename, sizeof(filename)) == EOK) {
				filename_found = true;
			}
			ext4_fs_put_inode_ref(&root_ref);
		}
	}
	
	// Add filename if found
	if (filename_found && strlen(filename) > 0) {
		char temp[512];
		snprintf(temp, sizeof(temp), "%s [%s]", path_info, filename);
		strncpy(path_info, temp, max_len - 1);
		path_info[max_len - 1] = '\0';
	}
}
...
{
	return ext4_balloc_alloc_block(inode_ref, goal, blockp, 1);
}

static ext4_fsblk_t ext4_new_meta_blocks(struct ext4_inode_ref *inode_ref,
					 ext4_fsblk_t goal,
					 uint32_t flags __unused, uint32_t n_blks,
					 uint32_t *count, int *errp)
{
	ext4_fsblk_t block = 0;

	if (n_blks > 1) {
		// We allocate multiple blocks at once.
		*errp = ext4_balloc_alloc_block(inode_ref, goal, &block, n_blks);
					    ...
					    uint32_t flags)
{
	...
	newblock = ext4_new_meta_blocks(inode_ref, goal, flags, 1, NULL, err);
	return newblock;
}
			  ...
			  struct ext4_extent_path *path)
{
	if (path->block.lb_id){
		...
		ext4_mark_buffer_dirty_inode(path->block.buf, &inode_ref->vfs_inode);
	} else {
		inode_ref->dirty = true;
		ext4_mark_inode_dirty(inode_ref);
	}
...
#ifdef MKFS
	err = ext4_trans_block_get(inode_ref->fs->bdev, bh, pblk, NULL);
#else
	err = ext4_trans_block_get(inode_ref->fs->bdev, bh, pblk, NULL);
#endif
			       ...
			       bool *ins_right_leaf)
{
	...
	for (i = depth; i >= at; i--, npath_at--) {
		...
		if (i == depth) {
			...
			ext4_mark_buffer_dirty_inode(bh.buf, &inode_ref->vfs_inode);
		} else {
			   ...
			   struct ext4_extent *newext, int flags)
{
	...
	// Print extent insertion info
	ext4_ext_debug("");
	ext4_ext_debug("=== EXTENT INSERT (inode %u) ===", inode_ref->index);
	ext4_ext_debug("Inserting extent: [%u-%u] -> %llu (len=%u%s)",
		to_le32(newext->first_block),
		to_le32(newext->first_block) + ext4_ext_get_actual_len(newext) - 1,
		ext4_ext_pblock(newext),
		ext4_ext_get_actual_len(newext),
		ext4_ext_is_unwritten(newext) ? ", unwritten" : "");
...
out:
	if (ret != EOK) {
		ext4_ext_error("Extent insertion FAILED with error %d", ret);
		if (path)
			ext4_ext_drop_refs(inode_ref, path, 0);
		...
		ext4_ext_debug("Extent insertion SUCCESS");
		// Print the updated extent tree after successful insertion
		ext4_print_extent_tree(inode_ref, "EXTENT_INSERT_SUCCESS");
	}
	ext4_ext_debug("=====================================");
...
// TODO: Decouple secure daemon from file system logic, extent (buffer_extent).
int ext4_extent_get_blocks(struct ext4_inode_ref *inode_ref, ext4_lblk_t iblock,
			   uint32_t max_blocks, ext4_fsblk_t *result,
			   bool create, uint32_t *blocks_count, struct buffer_extent *be)
{
	struct ext4_extent_path *path = NULL;
	...
	// Print block access info
	ext4_ext_debug("=== EXTENT GET_BLOCKS (inode %u) ===", inode_ref->index);
	ext4_ext_debug("Request: block=%u, max_blocks=%u, create=%s",
		iblock, max_blocks, create ? "YES" : "NO");

	// Reset be for validation.
	if (be) {
		be->iblock = 0;
		be->lba = 0;
		be->nr = 0;
	...
	if (ex) {
		...
		if (IN_RANGE(iblock, ee_block, ee_len)) {
			...
			if (!ext4_ext_is_unwritten(ex)) {
				...
				ext4_ext_debug("Found existing extent: [%u-%u] -> %llu (using block %llu)",
					ee_block, ee_block + ee_len - 1, ee_start, newblock);
				goto out;
			}

			if (!create) {
				newblock = 0;
				ext4_ext_warn("Found unwritten extent but create=NO, returning 0");
				goto out;
			}
			...
			newblock = iblock - ee_block + ee_start;
			ext4_ext_debug("Converting unwritten extent: [%u-%u] -> %llu (converting %u blocks)",
				ee_block, ee_block + ee_len - 1, ee_start, zero_range);
	...
	ext4_ext_debug("No existing extent found, allocating new blocks: goal=%llu, requested=%u", goal, allocated);
	
	newblock = ext4_new_meta_blocks(inode_ref, goal, 0, allocated,
					&allocated, &err);
	if (!newblock) {
		ext4_ext_error("Block allocation FAILED");
		goto out2;
	}
	
	ext4_ext_debug("Allocated %u blocks starting at %llu", allocated, newblock);
	...
	ext4_ext_debug("Creating new extent: [%u-%u] -> %llu (len=%u)",
		to_le32(newex.first_block), 
		to_le32(newex.first_block) + allocated - 1,
		newblock, allocated);
		
	err = ext4_ext_insert_extent(inode_ref, &path, &newex, 0);
	if (err != EOK) {
		/* free data blocks we just allocated */
		ext4_ext_error("Extent insertion failed, freeing allocated blocks");
	...
	if (be) {
		be->iblock = ext4_extent_get_first_block(&newex);
		be->lba = ext4_ext_pblock(&newex);
		be->nr = ext4_ext_get_actual_len(&newex); // TOCHECK: What is different from ext4_ext_get_block_count?
	}

	goto ret; // Do not update be with ex.

out:
	/* fill buffer_extent containing iblock */
	if (be) {
		be->iblock = ext4_extent_get_first_block(ex);
		be->lba = ext4_ext_pblock(ex);
		be->nr = ext4_ext_get_actual_len(ex); // TOCHECK: What is different from ext4_ext_get_block_count?
		
		// Debug output for tracking buffer_extent values
		ext4_ext_debug("=== BUFFER_EXTENT DEBUG ===");
		ext4_ext_debug("Requested iblock: %u", iblock);
		ext4_ext_debug("Calculated newblock: %llu", newblock);
		ext4_ext_debug("buffer_extent: iblock=%u, lba=%llu, nr=%u", 
			be->iblock, be->lba, be->nr);
		ext4_ext_debug("Expected calc: %u - %u + %llu = %llu", 
			iblock, be->iblock, be->lba, (uint64_t)(iblock - be->iblock + be->lba));
		ext4_ext_debug("===========================");
	}
...
ret:
	...
	// ext4_ext_debug("get_blocks SUCCESS: returning block=%llu, count=%u", newblock, allocated);
	// ext4_ext_debug("=====================================");

out2:
	if (err != EOK) {
		ext4_ext_error("get_blocks FAILED with error %d", err);
		ext4_ext_error("=====================================");
	}
...
// Extent tree debugging functions
#ifdef PRINT_EXT4_EXT_TREE

#ifdef EXT4_EXT_LOG_TO_FILE
/* File logging helper functions */
static FILE* ext4_ext_get_log_file(uint32_t inode_num)
{
	static char filename[256];
	snprintf(filename, sizeof(filename), "/tmp/extent_%u.log", inode_num);
	return fopen(filename, "a");
}

static void ext4_ext_log_to_file(uint32_t inode_num, const char* format, ...)
{
	FILE* logfile = ext4_ext_get_log_file(inode_num);
	if (logfile) {
		va_list args;
...
static void ext4_ext_log_timestamp(uint32_t inode_num)
{
	FILE* logfile = ext4_ext_get_log_file(inode_num);
	if (logfile) {
...
#define ext4_ext_dual_debug(inode_num, format, ...) \
	do { \
		ext4_ext_log_to_file(inode_num, format, ##__VA_ARGS__); \
	} while(0)
	// do { \
	// 	oxb_debug(format, ##__VA_ARGS__); \
	// 	ext4_ext_log_to_file(inode_num, format, ##__VA_ARGS__); \
	// } while(0)
#else
#define ext4_ext_dual_debug(inode_num, format, ...) oxb_debug(format, ##__VA_ARGS__)
#define ext4_ext_log_timestamp(inode_num) do {} while(0)
#endif

static void ext4_print_extent_header(struct ext4_extent_header *eh, int depth, const char *prefix, uint32_t inode_num, ext4_fsblk_t block_addr)
{
	ext4_ext_dual_debug(inode_num, "%sExtent Header (depth %d):", prefix, depth);
	ext4_ext_dual_debug(inode_num, "%s  baddr: %llu", prefix, block_addr);
	ext4_ext_dual_debug(inode_num, "%s  magic: 0x%04x", prefix, to_le16(eh->magic));
	ext4_ext_dual_debug(inode_num, "%s  entries: %d / %d", prefix, to_le16(eh->entries_count), to_le16(eh->max_entries_count));
	ext4_ext_dual_debug(inode_num, "%s  depth: %d", prefix, to_le16(eh->depth));
	ext4_ext_dual_debug(inode_num, "%s  generation: %u", prefix, to_le32(eh->generation));
}

static void ext4_print_extent_entry(struct ext4_extent *ex, int index, const char *prefix, uint32_t inode_num)
{
	ext4_ext_dual_debug(inode_num, "%sExtent %d: [%u-%u] -> [%llu-%llu] (len=%u%s)", prefix, index,
		to_le32(ex->first_block),
		to_le32(ex->first_block) + ext4_ext_get_actual_len(ex) - 1,
		ext4_ext_pblock(ex),
		ext4_ext_pblock(ex) + ext4_ext_get_actual_len(ex) - 1,
		ext4_ext_get_actual_len(ex),
		ext4_ext_is_unwritten(ex) ? ", unwritten" : "");
}

static void ext4_print_extent_index(struct ext4_extent_index *ix, int index, const char *prefix, uint32_t inode_num)
{
	ext4_ext_dual_debug(inode_num, "%sIndex %d: [%u+] -> %llu", prefix, index,
		to_le32(ix->first_block),
		ext4_idx_pblock(ix));
}

static void ext4_print_extent_tree_recursive(struct ext4_inode_ref *inode_ref, 
					struct ext4_extent_header *eh, 
					...
					int depth, const char *prefix)
{
	...
	ext4_print_extent_header(eh, depth, prefix, inode_ref->index, block_addr);
	
	if (to_le16(eh->depth) == 0) {
		// Leaf node - print extents
		struct ext4_extent *ex = EXT_FIRST_EXTENT(eh);
		for (i = 0; i < to_le16(eh->entries_count); i++) {
			ext4_print_extent_entry(&ex[i], i, prefix, inode_ref->index);
		}
	} else {
		// Index node - print indices and recurse
		struct ext4_extent_index *ix = EXT_FIRST_INDEX(eh);
		for (i = 0; i < to_le16(eh->entries_count); i++) {
			ext4_print_extent_index(&ix[i], i, prefix, inode_ref->index);
			
			// Recursively print child nodes
			snprintf(new_prefix, sizeof(new_prefix), "%s  ", prefix);
			
			struct ext4_block child_block = EXT4_BLOCK_ZERO();
			ext4_fsblk_t child_addr = ext4_idx_pblock(&ix[i]);
			
			int rc = ext4_trans_block_get(inode_ref->fs->bdev, &child_block, child_addr, NULL);
			if (rc == EOK) {
				struct ext4_extent_header *child_eh = ext_block_hdr(&child_block);
				ext4_print_extent_tree_recursive(inode_ref, child_eh, child_addr, depth + 1, new_prefix);
				ext4_block_set(inode_ref->fs->bdev, &child_block);
			} else {
				ext4_ext_dual_debug(inode_ref->index, "%s  Error reading child block at %llu", prefix, child_addr);
			}
		}
	}
}

void ext4_print_extent_tree(struct ext4_inode_ref *inode_ref, const char *operation)
{
	// char path_info[512];
	// ext4_get_inode_path_info(inode_ref, path_info, sizeof(path_info));
	
#ifdef EXT4_EXT_LOG_TO_FILE
	ext4_ext_log_timestamp(inode_ref->index);
#endif
	
	ext4_ext_dual_debug(inode_ref->index, "");
	ext4_ext_dual_debug(inode_ref->index, "=== EXTENT TREE DEBUG: %s ===", operation);
	// ext4_ext_dual_debug(inode_ref->index, "File: %s", path_info);
	
	struct ext4_extent_header *root_eh = ext_inode_hdr(inode_ref->inode);
	
	// Print basic inode info
	ext4_ext_dual_debug(inode_ref->index, "Inode Details:");
	ext4_ext_dual_debug(inode_ref->index, "  Inode number: %u", inode_ref->index);
	ext4_ext_dual_debug(inode_ref->index, "  Size: %llu bytes", ext4_inode_get_size(&inode_ref->fs->sb, inode_ref));
	ext4_ext_dual_debug(inode_ref->index, "  Blocks: %llu", ext4_inode_get_blocks_count(&inode_ref->fs->sb, inode_ref->inode));
	ext4_ext_dual_debug(inode_ref->index, "  Has extents: %s", 
		ext4_inode_has_flag(inode_ref->inode, EXT4_INODE_FLAG_EXTENTS) ? "Yes" : "No");
	
	if (!ext4_inode_has_flag(inode_ref->inode, EXT4_INODE_FLAG_EXTENTS)) {
		ext4_ext_dual_debug(inode_ref->index, "  (Using indirect blocks, not extents)");
		ext4_ext_dual_debug(inode_ref->index, "========================================");
		return;
	}
	
	// Validate extent header before printing tree structure
	uint16_t magic = to_le16(root_eh->magic);
	uint16_t entries = to_le16(root_eh->entries_count);
	uint16_t max_entries = to_le16(root_eh->max_entries_count);
	uint16_t depth = to_le16(root_eh->depth);
	
	if (magic != EXT4_EXTENT_MAGIC) {
		ext4_ext_dual_debug(inode_ref->index, "  Invalid extent header magic (0x%04x, expected 0x%04x)", magic, EXT4_EXTENT_MAGIC);
		ext4_ext_dual_debug(inode_ref->index, "========================================");
		return;
	}
	
	if (max_entries == 0) {
		ext4_ext_dual_debug(inode_ref->index, "  Invalid extent header: max_entries_count is 0");
		ext4_ext_dual_debug(inode_ref->index, "========================================");
		return;
	}
	
	if (entries > max_entries) {
		ext4_ext_dual_debug(inode_ref->index, "  Invalid extent header: entries_count (%u) > max_entries_count (%u)", entries, max_entries);
		ext4_ext_dual_debug(inode_ref->index, "========================================");
		return;
	}
	
	if (depth > 5) { // Reasonable depth limit
		ext4_ext_dual_debug(inode_ref->index, "  Invalid extent header: depth too large (%u)", depth);
		ext4_ext_dual_debug(inode_ref->index, "========================================");
		return;
	}
	
	ext4_ext_dual_debug(inode_ref->index, "");
	ext4_ext_dual_debug(inode_ref->index, "Extent Tree Structure:");
	ext4_print_extent_tree_recursive(inode_ref, root_eh, 0, 0, "");
