					...
					struct ext4_block *block)
{
	...
	ext4_mark_buffer_dirty_inode(block->buf, &inode_ref->vfs_inode);
}
...
static int ext4_xattr_try_alloc_block(struct ext4_inode_ref *inode_ref)
{
	...
	if (!xattr_block) {
		...
		ret = ext4_balloc_alloc_block(inode_ref, goal, &xattr_block, 1);
		if (ret != EOK)
			goto Finish;
		    ...
		    struct ext4_xattr_list_entry *list, size_t *list_len)
{
	...
	if (xattr_block) {
#ifdef MKFS
		ret = ext4_trans_block_get(fs->bdev, &block, xattr_block, NULL);
#else
		ret = ext4_trans_block_get(fs->bdev, &block, xattr_block, NULL);
#endif
		if (ret != EOK)
				     ...
				     ext4_fsblk_t *orig_block, bool *allocated)
{
	...
	if (to_le32(header->h_refcount) > 1) {
		...
		ext4_mark_buffer_dirty_inode(new_block->buf, &inode_ref->vfs_inode);
...
#ifdef MKFS
	ret = ext4_trans_block_get(fs->bdev, &block, orig_xattr_block, NULL);
#else
	ret = ext4_trans_block_get(fs->bdev, &block, orig_xattr_block, NULL);
#endif
	if (ret != EOK)
