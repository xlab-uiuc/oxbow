			       ...
			       unsigned int nbits)
{
	...
	for (pos = find_first_bit(buf, nbits); pos < nbits && ord;
		  ...
		  unsigned int nbits)
{
	...
	for_each_set_bit (oldbit, src, nbits) {
		...
		if (n < 0 || w == 0)
			...
			set_bit(bitmap_ord_to_pos(_new, n % w, nbits), dst);
	}
}
...
static int __reg_op(unsigned long *bitmap, unsigned int pos, int order,
		    int reg_op)
{
 ...
 */
unsigned int bitmap_from_u32array(unsigned long *bitmap, unsigned int nbits,
