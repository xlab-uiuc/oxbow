...
inline void time_stats_stop(struct time_stats *ts)
{
	...
	double start_sec = (double)(ts->time_start.tv_sec * 1000000000.0 +
				    (double)ts->time_start.tv_nsec) /
			   1000000000.0;
	double end_sec =
		(double)(end.tv_sec * 1000000000.0 + (double)end.tv_nsec) /
		1000000000.0;
...
#else
	fprintf(stderr, "%s\n", msg);
	fprintf(stderr, "\tavg: %.3f msec (%.2f usec)\n", avg * 1000.0,
		avg * 1000000.0);
	// fprintf(stderr, "\tmin: %.3f msec (%.2f usec)\n", min * 1000.0, min * 1000000.0);
	// fprintf(stderr, "\tmax: %.3f msec (%.2f usec)\n", max * 1000.0, max * 1000000.0);
	// fprintf(stderr, "\tstd: %.3f msec (%.2f usec)\n", std * 1000.0, std * 1000000.0);
	// fprintf(stderr, "\t50 percentile    : %.3f msec (%.2f usec)\n", lat_50 * 1000.0, lat_50 * 1000000.0);
	// fprintf(stderr, "\t99 percentile    : %.3f msec (%.2f usec)\n", lat_99 * 1000.0, lat_99 * 1000000.0);
	// fprintf(stderr, "\t99.9 percentile  : %.3f msec (%.2f usec)\n", lat_99_9 * 1000.0, lat_99_9 * 1000000.0);
	// fprintf(stderr, "\t99.99 percentile : %.3f msec (%.2f usec)\n", lat_99_99 * 1000.0, lat_99_99 * 1000000.0);
	// fprintf(stderr, "\t99.999 percentile: %.3f msec (%.2f usec)\n", lat_99_999 * 1000.0, lat_99_999 * 1000000.0);
#endif
}
